...
func (_q *TaskQuery) QueryHistory() *TaskHistoryQuery {
	query := (&TaskHistoryClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *TaskQuery) WithHistory(opts ...func(*TaskHistoryQuery)) *TaskQuery {
	query := (&TaskHistoryClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *TaskQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Task, error) {
	...
	if query := _q.withHistory; query != nil {
		if err := _q.loadHistory(ctx, query, nodes,
			func(n *Task) { n.Edges.History = []*TaskHistory{} },
			func(n *Task, e *TaskHistory) { n.Edges.History = append(n.Edges.History, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *TaskQuery) loadHistory(ctx context.Context, query *TaskHistoryQuery, nodes []*Task, init func(*Task), assign func(*Task, *TaskHistory)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Task)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.TaskHistory(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(task.HistoryColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.task_history
		if fk == nil {
			return fmt.Errorf(`foreign-key "task_history" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "task_history" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
