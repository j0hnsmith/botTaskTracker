...
func (_q *TaskHistoryQuery) QueryTask() *TaskQuery {
...
func (_q *TaskHistoryQuery) FirstID(ctx context.Context) (id int, err error) {
	var ids []int
...
func (_q *TaskHistoryQuery) FirstIDX(ctx context.Context) int {
...
func (_q *TaskHistoryQuery) OnlyID(ctx context.Context) (id int, err error) {
	var ids []int
...
func (_q *TaskHistoryQuery) OnlyIDX(ctx context.Context) int {
...
func (_q *TaskHistoryQuery) IDs(ctx context.Context) (ids []int, err error) {
...
func (_q *TaskHistoryQuery) IDsX(ctx context.Context) []int {
	ids, err := _q.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (_q *TaskHistoryQuery) WithTask(opts ...func(*TaskQuery)) *TaskHistoryQuery {
	query := (&TaskClient{config: _q.config}).Query()
...
//		GroupBy(taskhistory.FieldAction).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.TaskHistory.Query().
//		Select(taskhistory.FieldAction).
//		Scan(ctx, &v)
...
func (_q *TaskHistoryQuery) loadTask(ctx context.Context, query *TaskQuery, nodes []*TaskHistory, init func(*TaskHistory), assign func(*TaskHistory, *Task)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*TaskHistory)
	for i := range nodes {
		if nodes[i].task_history == nil {
			continue
		}
		fk := *nodes[i].task_history
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "task_history" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *TaskHistoryQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(taskhistory.Table, taskhistory.Columns, sqlgraph.NewFieldSpec(taskhistory.FieldID, field.TypeInt))
	_spec.From = _q.sql
